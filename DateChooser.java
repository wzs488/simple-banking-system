 ...
 *@version 1.0
 *Date chooser
 *Reference: https://www.cnblogs.com/zhjsll/p/4383312.html
 */
...
public class DateChooser extends JPanel {
	...
	private void initPanel() {
		...
		monthPanel.setBorder(BorderFactory.createLineBorder(Color.WHITE));
	...
	public void register(final JComponent showDate) {
		...
		showDate.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent me) {
				if (showDate.isEnabled()) {
					showDate.setForeground(Color.GRAY);
					if (isShow) {
						hidePanel();
					} else {
						showPanel(showDate);
	...
	private class MyLabel extends JLabel implements Comparator<MyLabel>, MouseListener, MouseMotionListener {
		...
		protected void paintComponent(Graphics g) {
			...
			if (isSelected) {
				Stroke s = new BasicStroke(1.0f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_BEVEL, 1.0f,
